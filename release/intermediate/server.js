...
var Koa = require('koa');
var websocket = require('koa-easy-ws');
...
function load(path){
	return parse(fs__default["default"].readFileSync(path, 'utf-8'))
}
...
function allocate$6(series, exchanges){
	...
	for(let exchange of exchanges){
		...
		if(!candle){
			...
			candle.h = Decimal__default["default"].max(candle.h, price);
			candle.l = Decimal__default["default"].min(candle.l, price);
			candle.c = price;
...
function integrate$2(series, exchange){
	...
	if(candle){
		candle.h = Decimal__default["default"].max(candle.h, price);
		candle.l = Decimal__default["default"].min(candle.l, price);
		candle.v = Decimal__default["default"].sum(candle.v, volume);
		candle.n += 1;
...
function compose(token){
	...
	let stats = {
		marketcap: new Decimal__default["default"](0),
		volume: {
			day: new Decimal__default["default"](0),
			week: new Decimal__default["default"](0),
		},
		trustlines: 0
	};
	...
	if(candles.length > 0){
		...
		stats.marketcap = Decimal__default["default"].mul(stats.supply || 0, newestCandle.c);
		stats.volume = {
			day: Decimal__default["default"].sum(
				...candles
					.slice(candles.indexOf(yesterdaysCandle))
					.map(candle => candle.v)
			),
			week: Decimal__default["default"].sum(
				...candles
					.map(candle => candle.v)
			)
		};
	...
	for(let i=0; i<tokens.length; i++){
		...
		let combined = aligned
			.map(([{ id, ...stat }, candle]) => ({
				...stat,
				marketcap: candle
					? Decimal__default["default"].mul(stat.supply, candle.c).toString() 
					: '0'
			}))
		...
		for(let timeframe of Object.values(this.config.tokens.market.timeframes)){
			this.cache.stats.integrate(
				...
				},
				{
					...stats,
					marketcap: candle
						? Decimal__default["default"].mul(stats.supply, candle.c).toString()
						: '0',
				}
			);
		}
...
class HTTPRouter extends Router__default["default"]{
	constructor(ctx){
		super();
...
class WSManager{
	...
	ping(){
		for(let client of this.clients){
			if(!client.alive){
				client.socket.close();
				log$4.info(`client #${client.id} inactivity kick`);
				continue
			}
