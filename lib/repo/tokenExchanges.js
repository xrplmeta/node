...
export function init(){
	this.exec(
		...
		CREATE INDEX IF NOT EXISTS 
		"TokenExchangesLedger" ON "TokenExchanges" 
		("ledger");`
	)
}
...
export function* iter({base, quote, from, to, recent} = {}){
	let sql = `
		...
		FROM TokenExchanges
		INNER JOIN LedgerCaptures ON (LedgerCaptures."index" = TokenExchanges.ledger)
	`

	if(base || quote){
		sql += `WHERE "base" IS ${base} AND "quote" IS ${quote}` // null in prepared statements takes 10,000x longer for some fkin reason (it didn't a week ago)
	}else if(from || to){
		sql += `WHERE id >= @from AND id <= @to`
	}else if(recent){
		sql += `ORDER BY ledger DESC LIMIT @recent`
	}
