...
export default class{
	...
	dropIndices(){
		let indices = this.allv(`SELECT name FROM sqlite_master WHERE type == 'index'`)
	...
	enableQueryProfiling(){
		for(let fnName of ['get', 'all', 'run']){
			...
			this[fnName] = (sql, ...params) => {
				...
				log.debug(`${this.fileName} query (${timeInMs}ms): ${formatted}`, params)

				return res
			}
		}

		log.debug(`query profiling enabled`)
	}
}