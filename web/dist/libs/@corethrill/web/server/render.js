...
function callHook(vnode) {
	var ret = this.apply(vnode.state, arguments)

	if(m.hooks.hasPost(this.name)){
		ret = m.hooks.callPost(this.name, ret, arguments)
	}
...
function* tryRender(view, attrs, options, allowAwait) {
  ...
  function* setHooks(source, vnode) {
    ...
    if (source.oninit) {
      let ret = callHook.call(source.oninit, vnode, waitFor)
...
export default async (view, attrs, options) => {
...
export const sync = (view, attrs, options) => {
